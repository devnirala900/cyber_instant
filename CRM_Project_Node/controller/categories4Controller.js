...
const Categories_4 = require("../model/categories4Model");
...
const addCategories = catchAsyncError(async (req, res, next) => {
    ...
    if (!uid || !name || !number || !address) {
        ...
        const isExitsUid = await Categories_4.findOne({ uid })
        if (isExitsUid) {
            return next(new ErrorHandler(`uid is already isexits`,404))
        }
        else {
            const data = await Categories_4.create(info)
            if (data) {
                res.status(200).json({
                    _id: data._id,
                    uid: data.uid,
...
const getAllCategories = catchAsyncError(async (req, res, next) => {
    const data = await Categories_4.find()
    if (data) {
        res.status(200).json({
            data
        })
...
const getCategoriesById = catchAsyncError(async (req, res, next) => {
    ...
    if (!uid) {
        ...
        const data = await Categories_4.findOne({ uid })
        if (data) {
            res.status(200).json({
                categories1: data
            })
...
const updateCategoryById = catchAsyncError(async (req, res, next) => {
    ...
    const data = await Categories_4.findOne({ _id })
    if (data) {
        await Categories_4.findByIdAndUpdate(data._id, info, { new: true })
        res.status(200).json({
...
const deleteCategoriesById = catchAsyncError(async (req, res, next) => {
    ...
    const data = await Categories_4.findOne({ _id })
    if (data) {
        await Categories_4.findByIdAndDelete({ _id })
        res.status(200).json({
...
const searchCategories = catchAsyncError(async (req, res, next) => {
    ...
    const categories = await Categories_4.find(keyword)
    // .find(
    //     // { _id: { $ne: req.user._id } }
    // )
    res.send(categories)
